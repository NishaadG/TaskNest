...
const TaskListTable = ({ tableData }) => {
  ...
  return (
    <div className="overflow-x-auto p-0 rounded-lg mt-3">
        ...
        <tbody>
            {tableData.map((task) => (
            <tr key={task._id} className="border-gray-200">
                <td className="my-3 px-4 text-gray-500 text-[13px] line-clamp-1 overflow-hidden">{task.title}</td>
                <td className= "py-4 px-4">
                    <span className = {`px-2 py-1 text-xs rounded inline-block ${getStatusBadgeColor(task.status)}`}>{task.status}</span>
                </td>
                <td className ="">
                    <span className = {`px-2 py-1 text-xs rounded inline-block ${getPriorityBadgeColor(task.priority)}`}>{task.status}</span>
                </td>
